...
const LS_RECORDS = "pettyCash_records";
const LS_LALA = "pettyCash_lala";
const LS_CATS = "pettyCash_categories";
...
const TabButton = ({ active, onClick, children }) => (
  <button onClick={onClick} style={{
    padding: "10px 22px", background: active ? "#1a1a2e" : "transparent",
    color: active ? "#e8c97a" : "#888", border: "none",
    borderBottom: active ? "2px solid #e8c97a" : "2px solid transparent",
    cursor: "pointer", fontSize: "14px", fontFamily: "'Noto Serif TC', serif",
    fontWeight: active ? "700" : "400", transition: "all 0.2s", whiteSpace: "nowrap",
  }}>{children}</button>
);
...
const Badge = ({ children, color }) => (
  <span style={{
    ...
    background: color === "gold" ? "rgba(232,201,122,0.18)" : "rgba(100,180,255,0.15)",
    color: color === "gold" ? "#e8c97a" : "#64b4ff",
    border: `1px solid ${color === "gold" ? "rgba(232,201,122,0.4)" : "rgba(100,180,255,0.3)"}`,
  }}>{children}</span>
);

const StatCard = ({ label, value, sub, highlight }) => (
  <div style={{
    background: highlight ? "rgba(232,201,122,0.08)" : "rgba(255,255,255,0.04)",
    border: `1px solid ${highlight ? "rgba(232,201,122,0.35)" : "rgba(255,255,255,0.1)"}`,
    borderRadius: "10px", padding: "16px 20px", minWidth: "140px",
  }}>
    <div style={{ fontSize: "11px", color: "#888", marginBottom: "6px", letterSpacing: "0.5px" }}>{label}</div>
...
export default function App() {
  ...
  const [records, setRecords] = useState(() => loadLS(LS_RECORDS, INITIAL_DATA));
  const [categories, setCategories] = useState(() => loadLS(LS_CATS, DEFAULT_CATEGORIES));
  ...
  const [lalaRecords, setLalaRecords] = useState(() => loadLS(LS_LALA, []));
  ...
  const save = useCallback((r, l, c) => {
    localStorage.setItem(LS_RECORDS, JSON.stringify(r));
    localStorage.setItem(LS_LALA, JSON.stringify(l));
  ...
  const calcStats = (recs) => {
    const totalExpense = recs.reduce((s, r) => s + r.expense, 0);
    const totalIncome = recs.reduce((s, r) => s + r.income, 0);
    return { totalExpense, totalIncome, net: totalExpense - totalIncome };
  };
  ...
  const filtered = activeRecords.filter(r =>
    (filterCat === "全部" || r.category === filterCat) &&
    (searchText === "" || r.description.includes(searchText) || r.invoice.includes(searchText))
  );
  ...
  const handleAddRecord = () => {
    ...
    setRecords(updated); save(updated, lalaRecords, categories);
  ...
  const handleEdit = (rec) => {
    ...
    setEditingId(rec.id); setShowAddForm(true);
  };
  ...
  const handleAddCategory = () => {
    if (newCat && !categories.includes(newCat)) {
      ...
      setCategories(updated); save(records, lalaRecords, updated);
  ...
  const handleAddLala = () => {
    ...
    setLalaRecords(updated); save(records, updated, categories);
  ...
  const exportCSV = () => {
    ...
    const a = document.createElement("a"); a.href = URL.createObjectURL(blob); a.download = `零用金明細_${tab}.csv`; a.click();
  };
  ...
  const btnPrimary = { background: "linear-gradient(135deg, #e8c97a, #c9a84c)", color: "#1a1a2e", border: "none", padding: "8px 18px", borderRadius: "6px", cursor: "pointer", fontWeight: "700", fontSize: "13px", fontFamily: "'Noto Serif TC', serif" };
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#0d0d1a", color: "#e0e0e0", fontFamily: "'Noto Serif TC', serif" }}>
      {/* Header */}
      <div style={{ background: "linear-gradient(135deg, #1a1a2e 0%, #16213e 100%)", borderBottom: "1px solid rgba(232,201,122,0.3)", padding: "18px 24px", display: "flex", alignItems: "center", justifyContent: "space-between", position: "sticky", top: 0, zIndex: 100 }}>
        <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
          ...
          <div>
            ...
            <div style={{ fontSize: "11px", color: "#555" }}>114/11/17 – 115/02/11　|　資料儲存於本機瀏覽器</div>
          </div>
        </div>
        <div style={{ display: "flex", gap: "10px", alignItems: "center" }}>
          {saveMsg && <span style={{ fontSize: "12px", color: "#6de89a" }}>{saveMsg}</span>}
        </div>
      </div>
      ...
      <div style={{ background: "#13132a", borderBottom: "1px solid rgba(255,255,255,0.08)", padding: "0 24px", display: "flex", gap: "4px", overflowX: "auto" }}>
        {["紫焰", "終極", "LALA運費"].map(t =>
          <TabButton key={t} active={tab === t} onClick={() => setTab(t)}>
            {t === "紫焰" ? "🔴 紫焰有限公司" : t === "終極" ? "🔵 終極有限公司" : "📦 LALA運費儲值"}
          </TabButton>
        )}
      </div>
        ...
        {tab === "LALA運費" && (
          <div>
            ...
            <div style={{ background: "rgba(255,200,100,0.06)", border: "1px solid rgba(255,200,100,0.2)", borderRadius: "10px", padding: "14px 18px", marginBottom: "20px", fontSize: "13px", color: "#ccc" }}>
              ⚠️ <strong style={{ color: "#e8c97a" }}>注意：</strong>LALA 平台目前無公開 API，請手動輸入現金或信用卡預儲紀錄，系統將自動分類計算。
            </div>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(160px, 1fr))", gap: "12px" }}>
                  {[["日期","date","text"],["說明","description","text"],["發票/收據","invoice","text"],["金額","amount","number"]].map(([label, key, type]) => (
                    <div key={key}>
                      ...
                      <input type={type} style={inputStyle} value={lalaForm[key]} onChange={e => setLalaForm(p => ({ ...p, [key]: e.target.value }))} />
                    </div>
                  ))}
                  <div>
                    <label style={{ fontSize: "11px", color: "#888", display: "block", marginBottom: "4px" }}>付款方式</label>
                    <select style={inputStyle} value={lalaForm.payType} onChange={e => setLalaForm(p => ({ ...p, payType: e.target.value }))}>
                      <option>現金</option><option>信用卡預儲</option>
                    </select>
                  </div>
                </div>
                <div style={{ display: "flex", gap: "10px", marginTop: "16px" }}>
                  <button style={btnPrimary} onClick={handleAddLala}>儲存</button>
              ...
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "13px" }}>
                <thead>
                  <tr style={{ background: "rgba(255,255,255,0.06)" }}>
                    {["日期","說明","付款方式","金額","發票/收據","操作"].map(h => (
                      <th key={h} style={{ padding: "10px 12px", textAlign: "left", color: "#888", fontWeight: "600", borderBottom: "1px solid rgba(255,255,255,0.1)", whiteSpace: "nowrap" }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {lalaRecords.length === 0 && <tr><td colSpan={6} style={{ padding: "40px", textAlign: "center", color: "#555" }}>尚無紀錄，請新增</td></tr>}
                  {lalaRecords.map(r => (
            ...
            {lalaRecords.length > 0 && (
              <div style={{ marginTop: "24px", display: "grid", gridTemplateColumns: "1fr 1fr", gap: "16px", maxWidth: "500px" }}>
                {["現金", "信用卡預儲"].map(pt => {
                  const recs = lalaRecords.filter(r => r.payType === pt);
                  const total = recs.reduce((s, r) => s + r.amount, 0);
                  return (
                    <div key={pt} style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: "10px", padding: "16px" }}>
                      <div style={{ fontSize: "12px", color: "#888", marginBottom: "8px" }}>{pt}</div>
                      <div style={{ fontSize: "20px", fontWeight: "700", color: pt === "信用卡預儲" ? "#e8c97a" : "#6de89a" }}>{fmt(total)}</div>
                      <div style={{ fontSize: "11px", color: "#555", marginTop: "4px" }}>{recs.length} 筆</div>
                    </div>
                  );
                })}
              </div>
          ...
          <>
            ...
            <div style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: "10px", padding: "16px", marginBottom: "20px" }}>
              ...
              <div style={{ display: "flex", gap: "10px", flexWrap: "wrap" }}>
                ...
                {[...new Set(activeRecords.map(r => r.category))].map(cat => {
                  ...
                  return (
                    <div key={cat} style={{ background: filterCat === cat ? "rgba(232,201,122,0.15)" : "rgba(255,255,255,0.05)", borderRadius: "8px", padding: "8px 14px", cursor: "pointer", border: filterCat === cat ? "1px solid rgba(232,201,122,0.4)" : "1px solid transparent" }} onClick={() => setFilterCat(f => f === cat ? "全部" : cat)}>
            ...
            {showAddForm && (
              <div style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(232,201,122,0.2)", borderRadius: "10px", padding: "20px", marginBottom: "20px" }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(150px, 1fr))", gap: "12px" }}>
                  <div>
                    <label style={{ fontSize: "11px", color: "#888", display: "block", marginBottom: "4px" }}>日期</label>
                    <input style={inputStyle} value={form.date} onChange={e => setForm(p => ({ ...p, date: e.target.value }))} placeholder="115/02/11" />
                  </div>
                  <div>
                    <label style={{ fontSize: "11px", color: "#888", display: "block", marginBottom: "4px" }}>類別 <span style={{ fontSize: "10px", color: "#666" }}>(創命→終極)</span></label>
                    <select style={inputStyle} value={form.category} onChange={e => setForm(p => ({ ...p, category: e.target.value }))}>
                      {categories.map(c => <option key={c}>{c}</option>)}
                    </select>
                  </div>
                  <div>
                    <label style={{ fontSize: "11px", color: "#888", display: "block", marginBottom: "4px" }}>請款人</label>
                    <input style={inputStyle} value={form.requester} onChange={e => setForm(p => ({ ...p, requester: e.target.value }))} />
                  </div>
                  <div style={{ gridColumn: "span 2" }}>
                    <label style={{ fontSize: "11px", color: "#888", display: "block", marginBottom: "4px" }}>請款內容</label>
                    <input style={inputStyle} value={form.description} onChange={e => setForm(p => ({ ...p, description: e.target.value }))} />
                  </div>
                  <div>
                    <label style={{ fontSize: "11px", color: "#888", display: "block", marginBottom: "4px" }}>發票/收據號碼</label>
                    <input style={inputStyle} value={form.invoice} onChange={e => setForm(p => ({ ...p, invoice: e.target.value }))} />
                  </div>
                  <div>
                    <label style={{ fontSize: "11px", color: "#888", display: "block", marginBottom: "4px" }}>收入</label>
                    <input type="number" style={inputStyle} value={form.income} onChange={e => setForm(p => ({ ...p, income: e.target.value }))} placeholder="0" />
                  </div>
                  <div>
                    <label style={{ fontSize: "11px", color: "#888", display: "block", marginBottom: "4px" }}>支出</label>
                    <input type="number" style={inputStyle} value={form.expense} onChange={e => setForm(p => ({ ...p, expense: e.target.value }))} placeholder="0" />
                  </div>
                  <div style={{ display: "flex", alignItems: "flex-end" }}>
                    <div style={{ fontSize: "11px", color: "#666", background: "rgba(255,255,255,0.05)", padding: "8px 12px", borderRadius: "6px", width: "100%" }}>
                      歸屬：<strong style={{ color: form.category === "創命" ? "#64b4ff" : "#e8c97a" }}>{determineCompany(form.category)}</strong>
                    </div>
                  </div>
                </div>
                <div style={{ display: "flex", gap: "10px", marginTop: "16px" }}>
                  <button style={btnPrimary} onClick={handleAddRecord}>儲存</button>
            ...
            <div style={{ overflowX: "auto", borderRadius: "10px", border: "1px solid rgba(255,255,255,0.08)" }}>
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "12px" }}>
                <thead>
                  <tr style={{ background: "rgba(255,255,255,0.06)" }}>
                    {["序","日期","類別","請款人","請款內容","發票/收據","收入","支出","公司","操作"].map(h => (
                      <th key={h} style={{ padding: "10px 10px", textAlign: "left", color: "#888", fontWeight: "600", borderBottom: "1px solid rgba(255,255,255,0.1)", whiteSpace: "nowrap" }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                    ...
                    <tr key={r.id} style={{ borderBottom: "1px solid rgba(255,255,255,0.05)", background: i % 2 === 0 ? "transparent" : "rgba(255,255,255,0.015)" }}>
                      ...
                      <td style={{ padding: "9px 10px" }}>
                        <div style={{ display: "flex", gap: "6px" }}>
                          <button style={btnEdit} onClick={() => handleEdit(r)}>編輯</button>
                          <button style={btnDanger} onClick={() => handleDelete(r.id)}>刪</button>
                        </div>
                      </td>
                    </tr>
                  ))}
